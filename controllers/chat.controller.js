...
exports.sendMessage = (req, res) => {
    ...
    if (senderRole === 'user' && userId !== senderId) {
        return res.status(403).json({ message: "Unauthorized: Users can only send messages to their own chat." });
    }
...
exports.getMessages = (req, res) => {
    ...
    if (requesterRole === 'user' && userId !== requesterId) {
        return res.status(403).json({ message: "Unauthorized: You can only view your own chat messages." });
    }
    ...
    User.findById(userId, (userErr, user) => {
        if (userErr) {
            ...
            return res.status(500).json({ message: "Failed to retrieve user data for chat initialization." });
        }
        if (!user) {
            return res.status(404).json({ message: "User not found for chat initialization." });
        }

        const userWalletAddress = user.walletAddress;

        // MODIFIED: Pass requesterId (which will be admin's ID if admin is viewing)
        ChatMessage.ensureInitialMessages(userId, userWalletAddress, requesterId, (initErr, initResult) => {
            if (initErr) {
            ...
            ChatMessage.getMessagesByUserId(userId, (err, messages) => {
                ...
                ChatMessage.markMessagesAsRead(userId, requesterRole, (readErr, readResult) => {
                    if (readErr) {
                        console.error('Error marking messages as read after fetching:', readErr);
...
exports.getUsersWithUnreadMessages = (req, res) => {
    ...
    ChatMessage.getUsersWithUnreadMessagesForAdmin((err, users) => {
        if (err) {
            ...
            return res.status(500).json({ message: "Failed to retrieve unread messages list.", error: err.message });
        }
